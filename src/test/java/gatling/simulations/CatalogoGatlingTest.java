 ...
 */
public class CatalogoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all catalogos").get("/api/catalogos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new catalogo")
                        .post("/api/catalogos")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created catalogo").get("${new_catalogo_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created catalogo").delete("${new_catalogo_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Catalogo entity").exec(scn);
